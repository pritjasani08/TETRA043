...
function HistoryPage() {
  ...
  const filteredRows = useMemo(() => {
    ...
    return DETECTIONS.filter(
      (d) =>
        ...
        (q.trim() === "" ||
          `${d.animal} ${d.side} ${d.id} ${d.status}`.toLowerCase().includes(q.trim().toLowerCase())),
    );
  ...
  const stats = useMemo(() => {
    ...
    const boundaries = DETECTIONS.reduce((acc, curr) => {
       acc[curr.side] = (acc[curr.side] || 0) + 1;
       return acc;
    }, {} as Record<string, number>);
    const mostActiveBoundary = Object.entries(boundaries).sort((a, b) => b[1] - a[1])[0]?.[0] || "None";
          ...
          {rows.map((d) => (
            ...
            >
              ...
              <div className="p-5 pb-4 border-b border-border/50 bg-gradient-to-br from-surface to-white flex gap-4 items-start relative">
                <Badge className={cn("absolute top-4 right-4 text-[10px] font-bold px-2 py-0.5", getStatusColor(d.status))}>
                   {d.status}
                </Badge>
                ...
                <div className="flex-1 pt-1">
                  ...
                  <div className="flex flex-col gap-1 text-xs font-medium text-muted-foreground mt-1">
                    <span className="flex items-center gap-1.5"><MapPin className="size-3 text-primary/60" /> {d.side}</span>
                    <span className="flex items-center gap-1.5"><Calendar className="size-3 text-primary/60" /> {d.date} • {d.time}</span>
                  </div>
                </div>
              </div>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between pt-8 mt-4">
            <p className="text-sm font-medium text-muted-foreground">
              Showing <span className="text-foreground">{((currentPage - 1) * itemsPerPage) + 1}</span> to <span className="text-foreground">{Math.min(currentPage * itemsPerPage, filteredRows.length)}</span> of <span className="text-foreground">{filteredRows.length}</span> events
            </p>
