    ...
    try {
      ...
      if (endpoint.includes("/dashboard/summary")) {
        return {
          systemStatus: {
            state: "Secured",
            cameraStatus: "All Systems Normal",
            lastUpdated: "Just now",
          },
          metrics: [],
          charts: {
            dailyTrend: {
              labels: DAILY_TREND.map((d) => d.day),
              series: DAILY_TREND.map((d) => d.intrusions),
            },
            weeklyTrend: {
              labels: WEEKLY_ACTIVITY.map((d) => d.week),
              series: WEEKLY_ACTIVITY.map((d) => d.intrusions),
            },
            monthlyTrend: {
              labels: MONTHLY_ACTIVITY.map((d) => d.month),
              series: MONTHLY_ACTIVITY.map((d) => d.intrusions),
            },
