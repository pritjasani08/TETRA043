import { Activity, Clock, MapPin, AlertCircle, PhoneCall, Volume2, ShieldAlert } from "lucide-react";
          ...
          <div className="p-6 flex-1 bg-white sm:rounded-t-[2rem] -mt-6 relative shadow-[0_-8px_30px_rgba(0,0,0,0.12)]">
            ...
            <div className="grid grid-cols-2 gap-3 mb-8">
               <div className="p-3 bg-surface rounded-2xl border border-border">
                 <p className="text-[10px] uppercase font-bold text-muted-foreground mb-1 flex items-center gap-1.5"><Clock className="size-3" /> Time</p>
                 <p className="font-bold text-foreground">{event.time}</p>
                 <p className="text-xs font-medium text-muted-foreground">{event.date}</p>
               </div>
               <div className="p-3 bg-surface rounded-2xl border border-border">
                 <p className="text-[10px] uppercase font-bold text-muted-foreground mb-1 flex items-center gap-1.5"><MapPin className="size-3" /> Location</p>
                 <p className="font-bold text-foreground">{event.side}</p>
                 <p className="text-xs font-medium text-muted-foreground">Confidence: {event.confidence}%</p>
               </div>
            </div>
            
            {/* Timeline */}
            <h4 className="font-bold text-sm uppercase tracking-wider text-muted-foreground mb-4 border-b border-border pb-2">Action Timeline</h4>
            ...
            <h4 className="font-bold text-sm uppercase tracking-wider text-muted-foreground mb-4 border-b border-border pb-2">System Config</h4>
            <div className="space-y-3 mb-8">
              <div className="flex items-center justify-between p-3 rounded-xl bg-surface border border-border">
                 ...
                 <div className="flex items-center gap-2">
                   <Badge variant="outline" className="bg-white">Hindi</Badge>
                   <Button 
                     variant="outline" 
                     size="icon" 
                   ...
                   >
                     {isPlaying ? <Square className="size-3 fill-current" /> : <Play className="size-3 fill-current" />}
                   </Button>
                 </div>
              </div>
            </div>
